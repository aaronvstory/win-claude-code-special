...
# ================================
# Claude Code Right-Click Launcher
# ================================
...
$MY_GITHUB_TOKEN = ""        # Leave empty to load from registry
$MY_BRAVE_API_KEY = ""       # Leave empty to load from registry
$MY_EXA_API_KEY = ""         # Leave empty to load from registry
$MY_PERPLEXITY_API_KEY = ""  # Leave empty to load from registry

# Other optional configs
$MY_PERPLEXITY_MODEL = "sonar"
$MY_ALLOWED_DIRECTORIES = "C:\,D:\,E:\"  # Customize your allowed drives
...
# Load API tokens (from config or registry)
$tokens = @{
...
Write-Host "  Memory: ${NODE_MEMORY}MB heap (--max-old-space-size=$NODE_MEMORY)"
Write-Host
...
Write-Host ("  Command: node win-claude-code/runner.js --dangerously-skip-permissions")
Write-Host ("  Enhancements: Git Bash integration, path conversion, Windows optimizations")
Write-Host ("  Time:    {0}" -f (Get-Date))
Write-Host ('=' * 63)

# Run
& node --max-old-space-size=$NODE_MEMORY $cli --dangerously-skip-permissions
